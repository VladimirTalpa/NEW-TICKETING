...
const BOT_TOKEN = process.env.DISCORD_TOKEN || process.env.BOT_TOKEN || config.token;
const PRIMARY_GUILD_ID = "1259733702280216638";
...
async function buildLeaderboardImage(guild, topRows) {
  ...
  const height = headerH + rowsToRender * rowH + 38;
  const padding = 38;
  const canvas = createCanvas(width, height);
  const ctx = canvas.getContext("2d");
  const FONT_TITLE = "700 54px \"Bahnschrift\", \"Trebuchet MS\", \"Segoe UI\", sans-serif";
  const FONT_SUB = "500 24px \"Bahnschrift\", \"Trebuchet MS\", \"Segoe UI\", sans-serif";
  const FONT_ROW_MAIN = "700 30px \"Bahnschrift\", \"Trebuchet MS\", \"Segoe UI\", sans-serif";
  const FONT_ROW_META = "700 23px \"Bahnschrift\", \"Trebuchet MS\", \"Segoe UI Emoji\", \"Segoe UI Symbol\", sans-serif";
  ...
  const glow = ctx.createRadialGradient(width * 0.74, 108, 20, width * 0.74, 108, 360);
  glow.addColorStop(0, "rgba(255,255,255,0.28)");
  glow.addColorStop(1, "rgba(255,255,255,0)");
  ctx.fillStyle = glow;
  ctx.fillRect(0, 0, width, height);
  drawParticleField(ctx, width, height, ["255,255,255", "240,240,240", "255,62,62"], 62);
  ...
  ctx.shadowBlur = 16;
  ctx.fillText("Leaderboard of Mohgs Server", padding, 86);
  ctx.shadowBlur = 0;
  ...
  for (let i = 0; i < topRows.length; i++) {
    ...
    const rowX = 34;
    const rowW = width - 68;
    const rowHInner = rowH - 14;
    roundedRectPath(ctx, rowX, y, rowW, rowHInner, 16);
    ctx.fillStyle = i % 2 === 0 ? "#121212" : "#0d0d0d";
    ctx.fill();
    ctx.strokeStyle = i < 3 ? "#ffffff" : "#4e4e4e";
    ctx.lineWidth = i < 3 ? 2 : 1.4;
    ctx.stroke();

    const accentColors = ["#ff5858", "#ff8e59", "#ffb659"];
    if (i < 3) {
      roundedRectPath(ctx, rowX + 10, y + 10, 8, rowHInner - 20, 4);
      ctx.fillStyle = i === 0 ? "#ffffff" : accentColors[i];
      ctx.fill();
    }

    const avatarX = rowX + 34;
    const avatarY = y + 18;
    const avatarSize = 74;
    const rankX = avatarX + avatarSize + 28;
    const infoX = avatarX + avatarSize + 118;
    await drawNeonAvatar(ctx, row.avatarUrl, avatarX, avatarY, avatarSize, {
      outerStroke: i < 3 ? "#ffffff" : "#efefef",
      outerGlow: i < 3 ? "rgba(255,255,255,0.9)" : "rgba(255,255,255,0.66)",
    });
    drawAvatarOrbit(ctx, avatarX + avatarSize / 2, avatarY + avatarSize / 2, avatarSize / 2 + 10, {
      main: "rgba(255,255,255,0.9)",
      alt: "rgba(255,54,54,0.85)",
      count: 3,
    });
    ...
    await drawLineWithEmoji(ctx, {
      ...
      x: rankX,
      y: y + 46,
      font: "700 26px \"Bahnschrift\", \"Trebuchet MS\", \"Segoe UI\", sans-serif",
      color: "#f6f6f6",
      iconSize: 22,
      gap: 8,
    });
    ...
    const name = fitText(ctx, row.name, 420);
    ctx.fillStyle = "#f1f1f1";
    ctx.fillText(name, infoX, y + 48);

    const ticketText = `${row.tickets} tickets`;
    const ratingLabel = typeof row.rating === "number"
    ...
    const ticketPrefixWidth = 20 + 8 + ctx.measureText(ticketText).width + 34;
    await drawLineWithEmoji(ctx, {
      emojiId: EMOJI_STARS,
      text: ratingLabel,
      x: infoX + ticketPrefixWidth,
      y: y + 92,
      font: FONT_ROW_META,
      color: "#e7e7e7",
      iconSize: 20,
  ...
  if (topRows.length === 0) {
    roundedRectPath(ctx, 26, headerH + 8, width - 52, rowH - 12, 16);
    ...
    ctx.fillText("No helper data yet.", padding + 18, headerH + 76);
  }
...
async function buildProfileImage(user, rec) {
  ...
  async function drawStatBox(x, y, w, h, emojiId, label, value) {
    ...
    ctx.font = "700 44px \"Bahnschrift\", \"Trebuchet MS\", \"Segoe UI\", sans-serif";
    ctx.fillStyle = "#ffffff";
  ...
  ctx.font = "700 38px \"Segoe UI Emoji\", \"Segoe UI Symbol\", sans-serif";
  ctx.fillStyle = "#fff4f4";
  ctx.fillText(starsText, rightX + 420, r2 + boxH + 50);
  ctx.font = "600 34px \"Bahnschrift\", \"Trebuchet MS\", \"Segoe UI\", sans-serif";
  ctx.fillStyle = "#efefef";
  ctx.fillText(`User ID: ${String(user?.id || "-")}`, rightX + 34, r2 + boxH + 102);
...
/* ===================== LOGIN ===================== */
if (!BOT_TOKEN) {
  throw new Error("Missing bot token. Set DISCORD_TOKEN in .env or token in config.json");
}
